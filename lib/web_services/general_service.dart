...
class GeneralService {
  ...
  Future<dynamic> getCategory() async {
    ...
    ResponseModel responseModel =
        await _httpClient.getRequest(url: kGetCategoryURL);
  ...
  Future<dynamic> getNotification() async {
    ...
    ResponseModel responseModel =
        await _httpClient.getRequest(url: kGetNotificationsURL);
    if (responseModel.statusDescription.isNotEmpty &&
      ...
      for (int i = 0; i < notificationList.length; i++) {
        notifications.add(NotificationModel.fromJson(
            notificationList[i], responseModel.statusDescription));
      }
    } else {
      return responseModel.statusDescription;
    }
  ...
  Future<dynamic> getVendorsCategory() async {
    List<CategoryModel> category = <CategoryModel>[];
    ResponseModel responseModel =
        await _httpClient.getRequest(url: kGetVendorCategoryURL);
    print("Res: " "${responseModel.statusDescription.toString()}");
    if (responseModel.statusDescription.isNotEmpty &&
        responseModel.statusDescription == "Categories found" &&
        responseModel.data != null &&
        responseModel.data.length > 0) {
